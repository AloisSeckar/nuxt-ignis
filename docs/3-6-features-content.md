...
<PackagesReference :packages="[{ name: '@nuxt/content', version: '3.13.0' }]" />
...
This will reference [default config file](https://github.com/AloisSeckar/nuxt-ignis/blob/v0.5.3/core/app/utils/config/content.ts) to enable default `@nuxt-content` collection in your project. The extra step is required as it seems not possible to transfer the config file from the layer.
