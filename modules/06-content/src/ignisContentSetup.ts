...
export function ignisModuleDependencies(nuxtOptions: NuxtIgnisContentOptions) {
  ...
  if (options?.content?.enabled === true) {
    ...
    // @nuxt/content loads content.config.ts from rootDir via c12 during its own setup before Module Setup.
    // We ensure it exists here since using addTemplate would be too late.
    const rootDir = nuxtOptions.rootDir
    const userConfig = existsSync(join(rootDir, 'content.config.ts'))
    if (!userConfig) {
      writeFileSync(
        join(nuxtOptions.rootDir, 'content.config.ts'),
        [
